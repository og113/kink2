...
----------------------------------------------------------------------------------------------------------------------------*/
		...
		for (lint j = 0; j < p.N*p.NT; j++) {
			...
			if (p.Pot==3 && x==(p.N-1)) {
				...
				//#################################### OLD EQUATIONS MADE FOR D ################################
				dds.insert(2*j,2*(j-1)+1) 	= 1.0; //zero imaginary part of time derivative				
				mds(2*j) 				= -f(2*j);
				//#################################### TEST OF NEW EQUATIONS MADE FOR C ################################
				//dds.insert(2*j,2*(j-1)) 	= -1.0; //zero real part of time derivative				
				//dds.insert(2*j,2*j) 		= 1.0; //zero real part of time derivative	
				//mds(2*j) 					= -(f(2*j)-f(2*(j-1)));
				//#################################### NOTE NEITHER ARE WHAT IS DONE IN DL7 ################################
			}
			else if (t==0) {
